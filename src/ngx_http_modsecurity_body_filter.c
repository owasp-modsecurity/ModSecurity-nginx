...
void
ngx_postponed_write_handler (ngx_event_t *rev)
{
  ngx_http_request_t *r;
  r = rev->data;
  ngx_http_modsecurity_ctx_t *ctx = ngx_http_get_module_ctx (
      r, ngx_http_modsecurity_module);

  ngx_chain_t * write = ctx->temp_chain;
  ngx_chain_t *next = write->next;
  write->next = NULL;
  ngx_log_error (NGX_LOG_DEBUG, r->connection->log, 0,
		 "MDS go to write data chain %d\n", write);
  ctx->temp_chain = next;
  ngx_http_next_body_filter (r, write);
  //    if (n == NGX_AGAIN) {
  //                break;
  //    }
...
ngx_http_modsecurity_body_filter (ngx_http_request_t *r, ngx_chain_t *in)
{
  ...
  ngx_int_t ret;
  ngx_pool_t *old_pool;
  ngx_int_t response_processed = 0;
  ...
  for (chain = in; chain != NULL; chain = chain->next)
    {
      //allocate buffer for copy in postopone chain
      ngx_buf_t *copy_buf;
      //postpone chain
      ngx_chain_t* copy_chain;
      //if = 1, then response body chunk is last
      response_processed = chain->buf->last_buf;
      ...
      if (ret > 0)
	{
      ...
      // begin creation of new chain link
  ...
  if (response_processed)
    {
      ...
      // response processing is too slow, thus we processing them one-time
      ...
      else if (ret < 0)
	{
      ...
      // remove hijack from headers sending
      ctx->header_pt (r);
